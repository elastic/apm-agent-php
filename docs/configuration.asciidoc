...
and it is not guaranteed to be backwards compatible in future releases.
...
If this option is set to `true` then the agent creates spans for method executions
based on sampling aka statistical profiler.

Due to the nature of how sampling profilers work,
the duration of the inferred spans are not exact, but only estimations.
See <<config-profiling-inferred-spans-sampling-interval>>
to fine tune the trade-off between accuracy and overhead.
...
Note that effective minimum duration is also affected by the sampling interval
so it is
max(<<config-profiling-inferred-spans-min-duration>>, <<config-profiling-inferred-spans-sampling-interval>>)
...
The frequency at which stack traces are gathered in order to construct inferred spans.
The lower you set it, the more accurate the inferred spans durations will be.
...
(see <<config-profiling-inferred-spans-min-duration>>).
