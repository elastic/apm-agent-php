...
Configure settings via the `php.ini` file:
...
This string is used to ensure that only your {agent}s can send data to your APM Server.
You must have created the API key using the APM Servers {apm-server-ref-v}/api-key.html[command line tool].
...
The fallback configuration setting to control the logging level for the {agent}.
This setting occurs when the sink specific option is not set explicitly.
...
This string is used to ensure that only your {agent}s can send data to your APM Server.
Both the {agent}s and the APM Server have to be configured with the same secret token.
...
If the value is negative and invalid, the default value is used instead.
...
Your service name must only contain characters from the ASCII alphabet,
...
If the value is negative and invalid, the default value is used.
...
but not for context information, labels, or spans.
