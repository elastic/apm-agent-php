...
=== Controlling amount of APM data emitted by the agent
The following configuration settings control amount of APM data emitted by the agent
and they can be tuned to reduce the agent overhead on the monitored application,
network utilisation and amount of storage reuired by Elasticsearch.
...
so reducing the level for these logs will not reduced the amount of storage used Elasticsearch -
...
APM error events are created only for PHP errors which level passes
...
error event, which can reduce the overhead of the agent.
...
If instrumentation's name or any of its keywords match this configuration option
