...
This configuration option controls minimal duration that a span should have
for the agent to collect stack trace for it thus allowing to avoid the overhead
for the "quick" spans.

If the value is

- any positive value (e.g. `5ms`) - it limits stack trace collection to spans with duration
    equal to or greater than the given value (e.g. 5 milliseconds).
- `0` (or `0` with any duration units e.g. `0ms`) - it instructs the agent
    to collect stack traces for spans with any duration
- any negative value (e.g. `-1ms`) - it disable stack trace collection for spans completely
...
- any positive integer - the value is be used as the maximum number of frames
- `0` - it disables stack trace capture
- any negative integer - it instructs the agent to capture all the frames
