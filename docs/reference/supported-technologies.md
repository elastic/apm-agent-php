...
Elastic supports OpenTelemetry, which allows logs, metrics, and trace signal collection for many of these technologies. Consider using the [EDOT PHP SDK](https://www.elastic.co/docs/reference/opentelemetry/edot-sdks/php) for observability data so you continue to get the full power of Elastic's platform.
