...
final class SpanContextDestinationServiceDto
{
            ...
            function ($key, $value) use ($result): bool {
                switch ($key) {
                    case 'name':
                        ...
                        return true;
                    case 'resource':
                        $result->resource = self::assertValidKeywordString($value);
                        return true;
                    case 'type':
    ...
    {
        $this->assertMatches(new SpanContextDestinationServiceExpectations());
    }

    public function assertMatches(SpanContextDestinationServiceExpectations $expectations): void
    {
        self::assertSameKeywordStringExpectedOptional($expectations->name, $this->name);
        self::assertSameKeywordStringExpectedOptional($expectations->resource, $this->resource);
